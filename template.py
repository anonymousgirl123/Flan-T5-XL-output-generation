...
def llm_function(model, tokenizer, question, option_a, option_b, option_c, option_d):
    ...
    prompt = (
        ...
        "C. " + option_c + "\n"
        "D. " + option_d + "\n"
        "Answer with the correct option letter only (A, B, C, or D)."
    )
    ...
    final_output = generated_text.strip().upper()

    # Extract option letter from output
    if final_output in ["A", "B", "C", "D"]:
        return final_output
    
    # Extract first letter that matches an option
